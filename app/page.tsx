...
export default function Home() {
  ...
  const totalCPS =
    (knight.incrementAmount * knight.amount +
      farm.incrementAmount * farm.amount +
      village.incrementAmount * village.amount +
      kingdom.incrementAmount * kingdom.amount) *
    monster.cpsMultiplier *
    (ascensionLevel + 1);
  ...
  function addKnight(addValue: number) {
    setKnight({
      ...knight,
      amount: knight.amount + addValue,
      cost: Math.round(knight.cost * 1.5),
    });
  }
  ...
  function addFarm(addValue: number) {
    setFarm({
      ...farm,
      amount: farm.amount + addValue,
      cost: Math.round(farm.cost * 2),
    });
  }
  ...
  function addVillage(addValue: number) {
    setVillage({
      ...village,
      amount: village.amount + addValue,
      cost: Math.round(village.cost * 2.5),
    });
  }
  ...
  function addKingdom(addValue: number) {
    setKingdom({
      ...kingdom,
      amount: kingdom.amount + addValue,
      cost: Math.round(kingdom.cost * 3),
    });
  }
  ...
  function attackMonster(value: number) {
    ...
    if (monster.currentHealth - value <= 0) {
      setMonster({
        ...
        currentHealth: monster.maxHealth * 10,
        cpsMultiplier: monster.cpsMultiplier * 2,
      });
    }
  }
  ...
  function calculateAscension() {
    let tempNumber = 0;
    tempNumber = Math.floor(
      (totalScore - ascensionLevel * 10000000) / 10000000
    );
    ...
    switch (Value) {
      ...
      case 7: {
        if (score >= itemsCost.VilItem1 && !items.VilItem1) {
          ...
          setVillage({
            ...village,
            incrementAmount: village.incrementAmount * 2,
          });
      ...
      case 9: {
        if (score >= itemsCost.VilItem3 && !items.VilItem3) {
          ...
          setVillage({
            ...village,
            incrementAmount: village.incrementAmount * 3,
          });
      ...
      case 10: {
        if (score >= itemsCost.KingdomItem1 && !items.KingdomItem1) {
          ...
          setKingdom({
            ...kingdom,
            incrementAmount: kingdom.incrementAmount * 2,
          });
      ...
      case 12: {
        if (score >= itemsCost.KingdomItem3 && !items.KingdomItem3) {
          ...
          setKingdom({
            ...kingdom,
            incrementAmount: kingdom.incrementAmount * 3,
          });
  ...
  useEffect(() => {
    ...
    const storedMonsterCurrentHealth = localStorage.getItem(
      "monsterCurrentHealth"
    );
    ...
    setItems({
      Sword1: parsedItems[0],
      Sword2: parsedItems[1],
      Sword3: parsedItems[2],
      FarmItem1: parsedItems[3],
      FarmItem2: parsedItems[4],
      FarmItem3: parsedItems[5],
      VilItem1: parsedItems[6],
      VilItem2: parsedItems[7],
      VilItem3: parsedItems[8],
      KingdomItem1: parsedItems[9],
      KingdomItem2: parsedItems[10],
      KingdomItem3: parsedItems[11],
    });
  }, []);
      ...
      <div className="bg-backgroundMain">
        ...
        <div className="flex flex-row min-h-[50vh]">
          <div className="flex flex-col w-1/4">
            <div className="h-1/4">
              <button
                onClick={() => attackButttonClicked()}
                className="border border-purpleish rounded-r-md shadow-md w-1/2 py-1 px-4 text-lg bg-buttonMain text-white hover:bg-buttonClicked transition-colors duration-[250]"
              >
                <span className="flex flex-col">
                  <p>Attack</p>
                  <p className="text-lg bg-buttonClicked break-all">
                    {Math.round(score)}
                  </p>
                </span>
              </button>
            ...
            <div className="h-3/4 ">
              <div className="flex flex-col">
                <button
                  onClick={buyKnight}
                  className="text-lg w-1/2 mb-2 bg-buttonMain rounded-r-lg text-white hover:bg-buttonClicked"
                >
                  <span className="flex justify-between">
                    <span className="flex flex-col grow p-1">
                      <p>Knight</p>
                      <p className="text-lg bg-buttonClicked">
                        {knight.amount}
                      </p>
                    </span>
                    <p className="py-4 px-2 text-right text-lg rounded-r-lg bg-buttonClicked">
                      {knight.cost}
                    </p>
                  </span>
                </button>
                <button
                  onClick={buyFarm}
                  className="text-lg mb-2 bg-buttonMain rounded-r-lg text-white hover:bg-buttonClicked w-1/2 "
                >
                  <span className="flex justify-between">
                    <span className="flex flex-col grow p-1">
                      <p>Farm</p>
                    ...
                    <p className="py-4 px-2 text-lg rounded-r-lg bg-buttonClicked">
                      {farm.cost}
                    </p>
                  </span>
                </button>
            ...
            <div className="bg-gray-200 w-[128px] h-4 rounded">
              <div
                className={`bg-red-500 h-full rounded ${
                  monster.currentHealth === 0 ? "hidden" : ""
                }`}
                style={{
                  width: `${
          ...
          <div className="flex flex-col m-2 items-center w-1/2">
            <div className="flex flex-col border items-center p-4">
              ...
              <div className="grid grid-cols-3 grid-rows-4 mt-6 gap-4 ">
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={1}
                  itemFlag={items.Sword1}
                  itemName="Iron Sword"
                  itemCost="1000"
                  tooltipText="Each knight now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={2}
                  itemFlag={items.Sword2}
                  itemName="Gold Sword"
                  itemCost="10000"
                  tooltipText="Each knight now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={3}
                  itemFlag={items.Sword3}
                  itemName="Rune Sword"
                  itemCost="100,000"
                  tooltipText="Each knight now produces 3x as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={4}
                  itemFlag={items.FarmItem1}
                  itemName="Fertilizer"
                  itemCost="25000"
                  tooltipText="Each farm now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={5}
                  itemFlag={items.FarmItem2}
                  itemName="Horse"
                  itemCost="250,000"
                  tooltipText="Each farm now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={6}
                  itemFlag={items.FarmItem3}
                  itemName="Tractor"
                  itemCost="1,000,000"
                  tooltipText="Each farm now produces 3x as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={7}
                  itemFlag={items.VilItem1}
                  itemName="Community"
                  itemCost="50000"
                  tooltipText="Each village now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={8}
                  itemFlag={items.VilItem2}
                  itemName="Trading"
                  itemCost="500,000"
                  tooltipText="Each village now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={9}
                  itemFlag={items.VilItem3}
                  itemName="Education"
                  itemCost="2,500,000"
                  tooltipText="Each village now produces 3x as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={10}
                  itemFlag={items.KingdomItem1}
                  itemName="Hospitals"
                  itemCost="100,000"
                  tooltipText="Each kingdom now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={11}
                  itemFlag={items.KingdomItem2}
                  itemName="Diplomacy"
                  itemCost="1,000,000"
                  tooltipText="Each kingdom now produces twice as much gold"
                />
                <ItemsBtn
                  buyItemFunction={buyItem}
                  itemID={12}
                  itemFlag={items.KingdomItem3}
                  itemName="Expansion"
                  itemCost="10,000,000"
                  tooltipText="Each kingdom now produces 3x as much gold"
                />
              </div>
            </div>
          </div>
        ...
        <div className="">
          <button
            ...
            className="border border-purpleish rounded-md p-1 mx-2 text-lg bg-buttonMain text-white hover:bg-buttonClicked my-4"
          >
            Reset
          </button>
          <button
            onClick={() => ascend()}
            className="border border-purpleish rounded-md p-1 mx-2 text-lg bg-buttonMain text-white hover:bg-buttonClicked group"
          >
            <h1>Ascend</h1>
            <div className="absolute -mt-[2rem] ml-[4.2rem] bg-grayish p-2 rounded-sm shadow-md drop-shadow-md italic invisible group-hover:visible">
              <p>
                Gain {calculateAscension() - ascensionLevel} ascension levels
              </p>
              <p>Earn a new ascension level every 10 million gold</p>
              <p>Each ascension level gives you a +1x multiplier</p>
              <p>All clickers and upgrades will reset on ascension</p>
            </div>
          </button>
        </div>
      </div>
